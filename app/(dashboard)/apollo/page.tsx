...
} from "@components/apollo/ApolloSkeletonLoader";
import { ApolloEmptyState } from "@components/apollo/ApolloEmptyState";
import {
  ApolloStatsCards,
  ApolloStats,
} from "@components/apollo/ApolloStatsCards";
...
const EmailStatusBadge: React.FC<{ status: string | undefined }> = ({
  status,
}) => {
...
const Highlight: React.FC<{ text: string | undefined; highlight: string }> = ({
  text,
  highlight,
}) => {
  const safeText = text || "";
...
export default function ApolloPage() {
  ...
  const [analyzeResult, setAnalyzeResult] =
    useState<ApolloUrlAnalysisResponse | null>(null);
  ...
  const [searchResult, setSearchResult] =
    useState<ApolloContactsResponse | null>(null);
  const [searchError, setSearchError] = useState<string | null>(null);
  const [limit, setLimit] = useState(25);
  const [currentPage, setCurrentPage] = useState(1);
  const [expandedUnmappedCategories, setExpandedUnmappedCategories] = useState<
    Set<string>
  >(new Set());
  ...
  const [selectedContactUuids, setSelectedContactUuids] = useState<Set<string>>(
    new Set()
  );
  ...
  const handleCountContacts = async () => {
    ...
    try {
      ...
      const parsedIncludeCompanyNames =
        parseMultiValueInput(includeCompanyName);
      const parsedExcludeCompanyNames =
        parseMultiValueInput(excludeCompanyName);
      ...
      const includeCompanyNameParam =
        parsedIncludeCompanyNames.length > 0
      ...
      const countResult = await countContactsFromApolloUrl(url, {
        ...
        exclude_company_name:
          parsedExcludeCompanyNames.length > 0
            ? parsedExcludeCompanyNames
            : undefined,
        include_domain_list:
          parsedIncludeDomains.length > 0 ? parsedIncludeDomains : undefined,
        exclude_domain_list:
          parsedExcludeDomains.length > 0 ? parsedExcludeDomains : undefined,
      });
      ...
      if (countResult.success && countResult.data !== undefined) {
        ...
        setCountError(countResult.message || "Failed to count contacts");
        showToast(
          "Count Failed",
          countResult.message || "Failed to count contacts",
          "error"
        );
      }
    } catch (error) {
      console.error("Count error:", error);
      setCountError("An error occurred while counting contacts");
      showToast(
        "Count Failed",
        "An error occurred while counting contacts",
        "error"
      );
    } finally {
      setIsCounting(false);
      ...
      if (searchResult.success && searchResult.data) {
        ...
        if (searchResult.data.unmapped_categories.length > 0) {
          const unmappedNames = new Set(
            searchResult.data.unmapped_categories.map((c) => c.name)
          );
  ...
  const handleCountAnalyzeAndSearch = async () => {
    ...
    // Execute all 3 APIs in parallel
      ...
      try {
        ...
        if (analyzeResult.success && analyzeResult.data) {
          ...
          const categoryNames = new Set(
            analyzeResult.data.categories.map((c) => c.name)
          );
          setExpandedCategories(categoryNames);
          showToast(
            "URL Analyzed Successfully!",
            `Found ${
              analyzeResult.data.statistics.total_parameters || 0
            } parameters across ${
              analyzeResult.data.statistics.categories_used || 0
            } categories`,
            "success"
          );
          updateStats(true, 0);
          ...
          showToast(
            "Analysis Failed",
            analyzeResult.message || "Failed to analyze URL",
            "error"
          );
        }
      } catch (error) {
    ...
    const searchPromise = (async () => {
      try {
        ...
        if (searchResult.success && searchResult.data) {
          ...
          const contactsFound = searchResult.data.results.length;
          ...
          updateStats(true, contactsFound);
        } else {
          setSearchError(searchResult.message || "Failed to search contacts");
          showToast(
            "Search Failed",
            searchResult.message || "Failed to search contacts",
            "error"
          );
        }
      } catch (error) {
    ...
    // Wait for all promises to complete (but they update UI independently)
    await Promise.allSettled([analyzePromise, searchPromise, countPromise]);
  };
  ...
  const handleAnalyzeAndSearch = async () => {
    ...
    if (analyzeSuccess && searchSuccess) {
      showToast(
        ...
        `Found ${contactsFound} contact(s) and analyzed ${
          analyzeResult?.statistics.total_parameters || 0
        } parameters`,
        "success"
      );
      ...
      showToast(
        ...
        `Found ${
          ...
          analyzeResult?.statistics.categories_used || 0
        } categories`,
        "success"
      );
      ...
      showToast(
        "Operation Failed",
        "Both analysis and search failed. Please check the URL and try again.",
        "error"
      );
    }
  };
  ...
  const toggleSelectAll = () => {
    ...
    const allUuids = searchResult.results
      .filter((c) => c.uuid)
      .map((c) => c.uuid!);
    if (allUuids.length === 0) return;

    const allSelected = allUuids.every((uuid) =>
      selectedContactUuids.has(uuid)
    );
    if (allSelected) {
      setSelectedContactUuids(new Set());
    } else {
  ...
  const isSelectAllChecked = searchResult
    ? searchResult.results
        .filter((c) => c.uuid)
        .every((c) => selectedContactUuids.has(c.uuid!))
    : false;
  const isSelectAllIndeterminate = searchResult
    ? searchResult.results
        .filter((c) => c.uuid)
        .some((c) => selectedContactUuids.has(c.uuid!)) && !isSelectAllChecked
    : false;
        ...
        return (
          <div className="contacts-table-name-cell">
            ...
            <div className="contacts-table-name-content">
              <p className="contacts-table-name-text">{contact.name || "-"}</p>
              {contact.email && (
                <p className="contacts-table-name-email">{contact.email}</p>
              )}
            </div>
          </div>
        );
      ...
      <Card>
        <CardContent className="apollo-tab-content">
          ...
          <div className="apollo-input-section">
            <div>
              <label className="apollo-input-label">Apollo.io URL</label>
              <div className="apollo-input-wrapper">
                <Input
                  ...
                  onKeyPress={(e) =>
                    e.key === "Enter" && handleAnalyzeAndSearch()
                  }
            ...
            <div className="apollo-advanced-options">
              <div>
                <label
                  htmlFor="apollo-include-company-name"
                  className="apollo-input-label"
                >
              ...
              <div>
                <label
                  htmlFor="apollo-exclude-company-name"
                  className="apollo-input-label"
                >
              ...
              <div>
                <label
                  htmlFor="apollo-include-domain-list"
                  className="apollo-input-label"
                >
                  Include Domains
                </label>
              ...
              <div>
                <label
                  htmlFor="apollo-exclude-domain-list"
                  className="apollo-input-label"
                >
                  Exclude Domains
                </label>
            ...
            <Button
              ...
              disabled={isAnalyzing || loading || isCounting || !url.trim()}
              className="apollo-unified-button"
            >
              {isAnalyzing || loading || isCounting ? (
                <>
          ...
          {isCounting ? (
            ...
            <div className="apollo-count-banner apollo-count-banner--error">
              <div className="apollo-count-banner__content">
              ...
              <div className="apollo-count-banner__content">
                ...
                <div className="apollo-count-banner__text">
                  ...
                  <span className="apollo-count-banner__label">
                    contact{contactCount !== 1 ? "s" : ""} found matching your
                    criteria
                  </span>
                </div>
              </div>
          ...
          {/* Analyze & Search Results - Split Layout */}
          {(isAnalyzing ||
            loading ||
            analyzeResult ||
            searchResult ||
                ...
                {isAnalyzing ? (
                  ...
                  <div className="apollo-results">
                    ...
                    <div className="apollo-section">
                      <h3 className="apollo-section-subtitle">
                        Parameter Categories
                      </h3>
                          ...
                          >
                            ...
                            >
                              <div className="apollo-category-header-content">
                                ...
                                <span className="apollo-category-name">
                                  {category.name}
                                </span>
                                <span className="apollo-category-count">
                                  ({category.total_parameters} parameter
                                  {category.total_parameters !== 1 ? "s" : ""})
                                </span>
                              </div>
                            </button>
                            ...
                            {expandedCategories.has(category.name) && (
                              <div className="apollo-category-content">
                                {category.parameters.map((param, idx) => (
                                  <div
                                    key={`${category.name}-param-${
                                      param.name || idx
                                    }`}
                                    className="apollo-parameter-item"
                                  >
                                    <div className="apollo-parameter-name">
                                      {param.name}
                                    </div>
                    ...
                    {/* Parameter Mapping Summary - Show if Search completed */}
                    {searchResult && (
                      <div className="apollo-section">
                        <div className="apollo-mapping-summary">
                          <h4 className="apollo-mapping-summary-title">
                          ...
                          <div className="apollo-mapping-stats">
                            <div className="apollo-mapping-stat">
                              <div className="apollo-mapping-stat-value apollo-mapping-stat-value--primary">
                                {
                                  searchResult.mapping_summary
                                    .total_apollo_parameters
                                }
                              </div>
                              <div className="apollo-mapping-stat-label">
                                Total Parameters
                              </div>
                            </div>
                            <div className="apollo-mapping-stat">
                          ...
                          {searchResult.mapping_summary.mapped_parameter_names
                            .length > 0 && (
                            <div className="apollo-mapping-tags">
                              ...
                              <div className="apollo-mapping-tags-list">
                                {searchResult.mapping_summary.mapped_parameter_names.map(
                                  (name, idx) => (
                                    <span
                ...
                {loading ? (
                  ...
                  <div className="apollo-results">
                        ...
                        {searchResult.unmapped_categories.length > 0 && (
                          <div className="apollo-section">
                            <h4 className="apollo-section-subtitle">
                              Unmapped Parameters
                            </h4>
                            <div className="apollo-unmapped-list">
                              {searchResult.unmapped_categories.map(
                                (category, catIdx) => (
                                  <div
                                    key={`unmapped-category-${
                                  ...
                                  >
                                    ...
                                    >
                                      <div className="apollo-unmapped-header-content">
                                        ...
                                        <span className="apollo-unmapped-name">
                                          {category.name}
                                        </span>
                                        <span className="apollo-unmapped-count">
                                          ({category.total_parameters} parameter
                                          {category.total_parameters !== 1
                                            ? "s"
                                            : ""}
                                          )
                                        </span>
                                      ...
                                      <div className="apollo-unmapped-content">
                                        {category.parameters.map((param, idx) => (
                                          <div
                                            ...
                                            className="apollo-unmapped-parameter"
                                          >
                            ...
                            <Table responsive>
                              <TableHeader>
                                <TableRow>
                                  ...
                                  <TableHead className="table-head-actions">
                                    Actions
                                  </TableHead>
                                </TableRow>
                              </TableHeader>
                              <TableBody>
                                {searchResult.results.map((contact, index) => {
                                  const isSelected = contact.uuid
                                    ? selectedContactUuids.has(contact.uuid)
                                    : false;
                                  return (
                                    <TableRow
                                      ...
                                      onClick={(e) => {
                                        if (
                                          (e.target as HTMLElement).closest(
                                            ".checkbox-input-wrapper, .checkbox-input, .checkbox-box"
                                          )
                                        ) {
                                          return;
                                        }
                                        ...
                                        <Tooltip content="Edit contact">
                                          <Button
                                            ...
                                            onClick={(e) => {
                                              e.stopPropagation();
                                              alert(
                                                "Edit functionality is not supported by the current API."
                                              );
                                            }}
                        ...
                        {(searchResult.next ||
                          ...
                          <div className="apollo-pagination">
                            <div className="apollo-pagination-info">
                              Showing <strong>{searchResult.results.length}</strong>{" "}
                              contact(s)
                              {searchResult.next && (
          ...
          {/* Empty State - Show when no operations, no data, no errors */}
          {!isAnalyzing &&
            !loading &&
            !isCounting &&
            !analyzeResult &&
